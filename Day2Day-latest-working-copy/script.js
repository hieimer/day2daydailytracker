...
const formSubmitButton = addForm.querySelector(".create-task-button");
...
function showAddForm() {
  const shouldRefreshTasks = editingTaskId && isEditingTaskDirty();

  if (!saveEditingTaskIfDirty()) {
    return;
  }
  ...
  formSubmitButton.textContent = "Create Task";
  updateFormSubmitState();
  taskTitleInput.focus();
}

function showEditForm(taskId, row) {
  const shouldRefreshTasks = editingTaskId && editingTaskId !== taskId && isEditingTaskDirty();

  if (editingTaskId !== taskId && !saveEditingTaskIfDirty()) {
    return;
  }

  if (editingTaskId === taskId && !addForm.classList.contains("hidden")) {
    return;
  }

  if (shouldRefreshTasks) {
    renderTasks();
    row = taskList.querySelector(`[data-task-id="${taskId}"]`);
  }
  ...
  editingTaskSnapshot = getTaskSnapshot(task);
  createdDateDetail.textContent = `Created ${formatDate(task.createdAt)}`;
  updateDueDateText();
  row.after(addForm);
  emptyState.classList.add("hidden");
  addPrompt.classList.toggle("hidden", currentView !== "tasks");
  addForm.classList.remove("hidden");
  detailMetaRow.classList.remove("hidden");
  formSubmitButton.textContent = "Save Task";
  updateFormSubmitState();
  taskTitleInput.focus();
}
...
function updateFormSubmitState() {
  const hasRequiredTitle = taskTitleInput.value.trim().length > 0;
  const canSubmit = editingTaskId ? hasRequiredTitle && isEditingTaskDirty() : hasRequiredTitle;

  formSubmitButton.disabled = !canSubmit;
  formSubmitButton.setAttribute("aria-disabled", String(!canSubmit));
}
...
function isEditingTaskDirty() {
  if (!editingTaskId || !editingTaskSnapshot) {
    return false;
  }

  return JSON.stringify(getFormValues()) !== JSON.stringify(editingTaskSnapshot);
}

function saveEditingTaskIfDirty() {
  if (!editingTaskId || !isEditingTaskDirty()) {
    return true;
  }
...
taskTitleInput.addEventListener("input", updateFormSubmitState);
taskDescriptionInput.addEventListener("input", updateFormSubmitState);
